...
as failed runtime-qualification evidence. None is used in the thesis.
The full rerun switches to SUMO 1.20.0, matching the archived experiment runtime,
with matching Python bindings and a fresh output root. All 15 models were restarted.
...
All 15 runs completed and passed stability; all ten sweeps passed preflight. The source experiment revision is `2c604e7`. The computer was an Apple M3 Pro (12 logical CPUs, 18 GiB), macOS 14.2.1 arm64, Python 3.11.14. The original experiment and helper timing records are packaged with the release evidence.

Stage wall times: training 491.8 s; selection 633.6 s; clean evaluation 585.7 s; deterministic diagnostics 34.9 s; overlapping sweep/random-loss/control batch 1,684.7 s; main analysis 232.5 s; matched baselines 65.4 s. Subsequent robustness/control wrapper times cover reuse and aggregation, not the original simulations. Precision sensitivity was run as a separate three-worker analysis batch. Do not sum overlapping worker durations as elapsed time or interpret these measurements as hardware benchmarks.

The audit returns WITHHOLD for both model families and all ten checkpoints. H3 is supported in all ten; H1 in three, H2 in two and H4 in two. H1 for GAT-Outage seed 46 is sensitive to four-decimal rounding. Paired probability DEF has seven positive intervals, two negative intervals and one interval crossing zero. Request-row controls are positive for GAT-Outage and negative for GAT. These findings supersede the v10 numerical conclusions.

Before the CMake commands above, obtain the exact source with:

```sh
git clone --branch v1_20_0 --depth 1 https://github.com/eclipse-sumo/sumo.git runs/runtime/sumo-1.20.0
```

Verify the source revision against `96efa4d36d6c9af50710015bb725fc51730b582a`. Build dependencies include CMake, Ninja, Xerces-C and PROJ. The runtime binary hash in the execution record identifies the exact binary used; rebuilding against different system libraries can change that hash.
