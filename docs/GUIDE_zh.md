...
> **历史版本说明：** 本指南主体记录 `story_freeze_v1` 的旧三幕叙事，保留用于审计，
> 不能作为最终 v4 结果引用。最终复现实验请以
> `docs/REPRODUCE_EXPERIMENTS.md`、`runs/dissertation_v4/` 和
> `docs/dissertation/04_results.md` 为准。v4 中只有 H3 在六个训练模型中一致成立；
> H1/H2 均不成立，H4 结果混合，且 stale-attention shift 在 seed 44 方向相反。
...
**v4 一句话结论**：更长的观测层断联会稳定提高 WAMSN，但注意力是否向过期节点
转移取决于训练 seed；实验没有证明 AoI 会降低忠实度，因此每个新训练的模型都
需要单独做新鲜度和忠实度验证。
